...
public class DataHandler {
    ...
    public Message<Integer> addTask(TaskDTO task) {
        ...
        String query = "INSERT INTO tasks (householdId, description, statusId, priorityId, ownerId) " +
                "VALUES (?, ?, ?, ?, ?)";
        try {
            DatabaseHelper.dbNumberSet queryResult = dbHelper.executeUpdateGeneratedKeys(query, task.getHouseholdId(),
                    task.getDescription(), task.getStatus().getId(), task.getPriorityId(), task.getUserId());

            if (queryResult.getFirst() > 0) {
                Logger.info("Task added successfully");
                message = new Message<>(MessageTypeEnum.SUCCESS, "Task added successfully"
                  ,queryResult.getSecond());
            } else {
                Logger.error("Failed to add task");
                message = new Message<>(MessageTypeEnum.ERROR, "Failed to add task");
            }
        } catch (SQLException e) {
    ...
    public Message<Integer> addUser(UserDTO user) {
        ...
        Message<Integer> message;
        try {
            DatabaseHelper.dbNumberSet queryResult = dbHelper.executeUpdateGeneratedKeys(query, user.getName(), user.getHousehold().getId());
            if (queryResult.getFirst() > 0) {
    ...
    public Message<List<DatabaseHelper.dbNumberSet>> getActiveTaskPerUser(int householdId) {
        ...
        try {
            List<DatabaseHelper.dbNumberSet> resultSet
              = dbHelper.executeSelect(query, DatabaseHelper.dbNumberSet.class, householdId);
            if (resultSet.isEmpty())
                return new Message<>(MessageTypeEnum.WARNING, "Could not find any users to assign tasks to");
            message = new Message<>(MessageTypeEnum.SUCCESS, "", resultSet);
        }
        catch (Exception e) {
            message  = new Message<>(MessageTypeEnum.ERROR, "Error fetching active tasks per user: "+ e.getMessage());
        }
        return message;
    }
}
