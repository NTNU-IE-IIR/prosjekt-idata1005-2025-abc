...
public class DataHandler {
    ...
    //TODO
    public void deleteUser(UserDTO selectedUser) {
        String query = "DELETE FROM users WHERE id = ?";
        try {
            int rowsAffected = dbHelper.executeUpdate(query, selectedUser.getId());
            if (rowsAffected > 0) {
                Logger.info("User deleted successfully: " + selectedUser.getName());
            } else {
                Logger.error("Failed to delete user: " + selectedUser.getName());
            }
        } catch (SQLException e) {
            Logger.error("Error deleting user: " + e.getMessage() +
                    " User: " + selectedUser.getName());
            System.out.println("Household id " + selectedUser.getHousehold().getId()
